...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/01/18 14:57:27 by vfil              #+#    #+#             */
/*   Updated: 2020/01/18 14:57:38 by vfil             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	swap_and_rotate(t_md5 *dt, int i, unsigned int tmp1, unsigned int tmp2)
{
	...
	dt->d = dt->c;
	dt->c = dt->b;
	dt->b = dt->b + LEFTROTATE((dt->a + tmp1 + g_md5_k[i] + dt->tmp[tmp2]),
		g_md5_r[i]);
	dt->a = z;
}

void	main_algorithm_loop(t_md5 *dt, int i, unsigned int tmp1,
	unsigned int tmp2)
{
	while (++i < 64)
	{
